      ...
      {privacy ? (
        <>
          ...
          <p>
            {tr(
              lang,
              "Google Analytics (G-Y7MJDDGWE8) wird nur nach Ihrer Zustimmung geladen. Sie können Ihre Auswahl jederzeit über Analyse-Einstellungen im Footer ändern. Werbepersonalisierung bleibt deaktiviert. Schriftarten werden über Systemschriften dargestellt. Das Formular ist noch nicht an einen Versanddienst angeschlossen; Anfragen werden nicht zugestellt und nicht in einer Anwendungsdatenbank gespeichert. Beim Versuch des Versands verarbeitet der Server die Formulardaten zur Prüfung.",
              "Google Analytics (G-Y7MJDDGWE8) loads only after your consent. You can change your choice through Analytics settings in the footer. Ad personalization remains disabled. The website uses system fonts. The form is not connected to a delivery service; inquiries are not delivered or saved in an application database. When submission is attempted, the server processes form data for validation.",
              "Google Analytics (G-Y7MJDDGWE8) завантажується лише після вашої згоди. Вибір можна змінити через налаштування аналітики внизу сторінки. Персоналізацію реклами вимкнено. Сайт використовує системні шрифти. Форму не підключено до сервісу надсилання: запити не доставляються й не зберігаються в базі застосунку. Під час спроби надсилання сервер обробляє дані для валідації.",
              "Google Analytics (G-Y7MJDDGWE8) загружается только после вашего согласия. Выбор можно изменить через настройки аналитики внизу страницы. Персонализация рекламы отключена. Сайт использует системные шрифты. Форма не подключена к сервису отправки: запросы не доставляются и не сохраняются в базе приложения. При попытке отправки сервер обрабатывает данные для валидации.",
            )}
          </p>
          <h2>
            {tr(
