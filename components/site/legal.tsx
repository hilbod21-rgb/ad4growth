      ...
      {privacy ? (
        <>
          ...
          <p>
            {tr(
              lang,
              "Google Analytics (G-Y7MJDDGWE8) wird nur nach Ihrer Zustimmung geladen. Sie können Ihre Auswahl jederzeit über Analyse-Einstellungen im Footer ändern. Werbepersonalisierung bleibt deaktiviert. Schriftarten werden über Systemschriften dargestellt. Formularanfragen werden durch Cloudflare verarbeitet und per E-Mail an AD4GROWTH zur Bearbeitung übermittelt. Die Angaben werden nicht in einer Anwendungsdatenbank gespeichert.",
              "Google Analytics (G-Y7MJDDGWE8) loads only after your consent. You can change your choice through Analytics settings in the footer. Ad personalization remains disabled. The website uses system fonts. Form inquiries are processed by Cloudflare and emailed to AD4GROWTH for a personal response. The details are not saved in an application database.",
              "Google Analytics (G-Y7MJDDGWE8) завантажується лише після вашої згоди. Вибір можна змінити через налаштування аналітики внизу сторінки. Персоналізацію реклами вимкнено. Сайт використовує системні шрифти. Cloudflare обробляє запити з форми й надсилає їх електронною поштою до AD4GROWTH для особистої відповіді. Дані не зберігаються в базі застосунку.",
              "Google Analytics (G-Y7MJDDGWE8) загружается только после вашего согласия. Выбор можно изменить через настройки аналитики внизу страницы. Персонализация рекламы отключена. Сайт использует системные шрифты. Cloudflare обрабатывает запросы из формы и отправляет их электронной почтой в AD4GROWTH для личного ответа. Данные не сохраняются в базе приложения.",
            )}
          </p>
          <h2>
            {tr(
